...
function drawSmooth(ctx,pts){
  ...
  if(pts.length===2){
    ...
    for(var i=0;i<pts.length-1;i++){
      ...
      cp1x=pts[i].x+(pts[i+1].x-pts[i].x)*0.4;
      cp1y=pts[i].y+(pts[i+1].y-pts[i].y)*0.4;
      cp2x=pts[i].x+(pts[i+1].x-pts[i].x)*0.6;
      cp2y=pts[i].y+(pts[i+1].y-pts[i].y)*0.6;
      if(i>0){
        cp1x=pts[i].x+(pts[i+1].x-pts[i-1].x)*0.2;
        cp1y=pts[i].y+(pts[i+1].y-pts[i-1].y)*0.2;
      }
      if(i<pts.length-2){
        cp2x=pts[i+1].x-(pts[i+2].x-pts[i].x)*0.2;
        cp2y=pts[i+1].y-(pts[i+2].y-pts[i].y)*0.2;
      }
...
var DRAW_SMOOTH_SRC=`function drawSmooth(ctx,pts){if(!pts||pts.length<2)return;ctx.beginPath();ctx.moveTo(pts[0].x,pts[0].y);if(pts.length===2){ctx.lineTo(pts[1].x,pts[1].y);}else{for(var i=0;i<pts.length-1;i++){var cp1x,cp1y,cp2x,cp2y;cp1x=pts[i].x+(pts[i+1].x-pts[i].x)*0.4;cp1y=pts[i].y+(pts[i+1].y-pts[i].y)*0.4;cp2x=pts[i].x+(pts[i+1].x-pts[i].x)*0.6;cp2y=pts[i].y+(pts[i+1].y-pts[i].y)*0.6;if(i>0){cp1x=pts[i].x+(pts[i+1].x-pts[i-1].x)*0.2;cp1y=pts[i].y+(pts[i+1].y-pts[i-1].y)*0.2;}if(i<pts.length-2){cp2x=pts[i+1].x-(pts[i+2].x-pts[i].x)*0.2;cp2y=pts[i+1].y-(pts[i+2].y-pts[i].y)*0.2;}ctx.bezierCurveTo(cp1x,cp1y,cp2x,cp2y,pts[i+1].x,pts[i+1].y);}}}`;
...
// ── Validation helpers ──────────────────────────────────────────────
function validateStrike(v){
  var n=parseFloat(v);
  if(v===""||v===null||v===undefined)return null; // optional
  if(isNaN(n))return "Strike must be a number";
  if(n<0||n>360)return "Strike must be 0–360°";
  return null;
}
function validateDip(v){
  var n=parseFloat(v);
  if(v===""||v===null||v===undefined)return null; // optional
  if(isNaN(n))return "Dip must be a number";
  if(n<0||n>90)return "Dip must be 0–90°";
  return null;
}
function validateSampleId(id,samples,editingIdx){
  if(!id||!id.trim())return "Sample ID is required";
  var dup=samples.findIndex(function(s,i){return s.id===id.trim()&&i!==editingIdx;});
  if(dup>=0)return "ID already used by sample #"+(dup+1);
  return null;
}
...
function scoreFeature(type, feat){
  if(type==="town"){
    var fields=[feat.name,feat.townType];
    var filled=fields.filter(function(f){return f&&f.trim();}).length;
    return {score:filled,max:fields.length};
  }
  if(type==="sample"){
    var fields=[feat.id,feat.rock,feat.description,feat.strike,feat.dip];
    var filled=fields.filter(function(f){return f!==undefined&&f!==null&&String(f).trim()!=="";}).length;
    return {score:filled,max:fields.length};
  }
  if(type==="geology"){
    var fields=[feat.rock,feat.formation,feat.period,feat.contact];
    var filled=fields.filter(function(f){return f&&f.trim();}).length;
    return {score:filled,max:fields.length};
  }
  if(type==="road"){
    var fields=[feat.name,feat.surface];
    var filled=fields.filter(function(f){return f&&f.trim();}).length;
    return {score:filled,max:fields.length};
  }
  if(type==="river"){
    var fields=[feat.name,feat.flow];
    var filled=fields.filter(function(f){return f&&f.trim();}).length;
    return {score:filled,max:fields.length};
  }
    ...
    <div style={{background:"#0d1a2a",border:"1px solid #2a4a6a",borderRadius:7,padding:9,marginTop:6}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
        ...
        <div style={{display:"flex",gap:3}}>
          {["dec","dms"].map(function(f){return(
            <button key={f} onClick={function(){setFmt(f);setLatStr("");setLonStr("");setErr("");}}
              style={{border:"none",borderRadius:3,padding:"2px 7px",fontSize:9,cursor:"pointer",background:fmt===f?"#f0c040":"#1a2a3a",color:fmt===f?"#000":"#888",fontWeight:"bold"}}>
              {f==="dec"?"Dec":"DMS"}
            </button>
          );})}</div>
      </div>
      <div style={{display:"flex",gap:4,marginBottom:4}}>
        <div style={{flex:1}}>
          ...
          <input value={latStr} onChange={function(e){setLatStr(e.target.value);setErr("");}} onKeyDown={function(e){if(e.key==="Enter")handle();}} placeholder={ph.lat} style={Object.assign({},INP,{marginBottom:0})}/>
        </div>
        <div style={{flex:1}}>
          <div style={{fontSize:9,color:"#666",marginBottom:2}}>Longitude</div>
          <input value={lonStr} onChange={function(e){setLonStr(e.target.value);setErr("");}} onKeyDown={function(e){if(e.key==="Enter")handle();}} placeholder={ph.lon} style={Object.assign({},INP,{marginBottom:0})}/>
        </div>
      </div>
      {err&&<div style={{fontSize:9,color:"#e74c3c",marginBottom:4}}>{err}</div>}
      <button onClick={handle} style={{width:"100%",background:"#1a4a2a",color:"#27ae60",border:"1px solid #27ae60",borderRadius:4,padding:"5px",fontSize:10,cursor:"pointer",fontWeight:"bold"}}>
        ✓ Place on Map & Pan
      </button>
    </div>
  );
...
// ── Edit panel for selected features ──────────────────────────────
  ...
  return(
    <div style={{background:"#12122e",border:"1px solid #1a3a5a",borderRadius:8,padding:10}}>
      ...
      <Field label="Town Name *">
        <input value={f.name||""} onChange={function(e){upd("name",e.target.value);}} placeholder="e.g. Ananamong" style={INP}/>
      </Field>
      <Field label="Settlement Type">
        <select value={f.townType||"Settlement"} onChange={function(e){upd("townType",e.target.value);}} style={SEL}>
          {TOWN_TYPES.map(function(t){return <option key={t}>{t}</option>;})}
        </select>
      </Field>
      <div style={{fontSize:9,color:"#555",fontFamily:"monospace",marginBottom:8}}>
        {f.lat&&f.lon?toDMS(f.lat,true)+" "+toDMS(f.lon,false):""}
      </div>
...
function SampleEditPanel({sample,allSamples,editingIdx,onSave,onDelete,onDeselect}){
  ...
  function validate(){
    var e={};
    var idErr=validateSampleId(f.id,allSamples,editingIdx);
    if(idErr)e.id=idErr;
    var sErr=validateStrike(f.strike);
    if(sErr)e.strike=sErr;
    var dErr=validateDip(f.dip);
    if(dErr)e.dip=dErr;
    setErrs(e);
  ...
  return(
    <div style={{background:"#12122e",border:"1px solid #5a1a1a",borderRadius:8,padding:10}}>
      ...
      <Field label="Sample ID *">
        <input value={f.id||""} onChange={function(e){upd("id",e.target.value);}} placeholder="e.g. UU/GS/GLG/25/57" style={INP}/>
        {errs.id&&<div style={{fontSize:9,color:"#e74c3c",marginTop:-3,marginBottom:4}}>{errs.id}</div>}
      </Field>
      <Field label="Rock Type">
        <select value={f.rock||"Shale"} onChange={function(e){upd("rock",e.target.value);}} style={SEL}>
          {ROCK_TYPES.map(function(r){return <option key={r}>{r}</option>;})}
        </select>
      </Field>
      <Field label="Field Description">
        <input value={f.description||""} onChange={function(e){upd("description",e.target.value);}} placeholder="e.g. dark grey, finely laminated" style={INP}/>
      </Field>
      <div style={{display:"flex",gap:6}}>
        <div style={{flex:1}}>
          <Field label="Strike (0–360°)">
            <input value={f.strike||""} onChange={function(e){upd("strike",e.target.value);}} placeholder="e.g. 045" style={INP}/>
            {errs.strike&&<div style={{fontSize:9,color:"#e74c3c",marginTop:-3,marginBottom:4}}>{errs.strike}</div>}
          </Field>
        </div>
        <div style={{flex:1}}>
          <Field label="Dip (0–90°)">
            <input value={f.dip||""} onChange={function(e){upd("dip",e.target.value);}} placeholder="e.g. 32" style={INP}/>
            {errs.dip&&<div style={{fontSize:9,color:"#e74c3c",marginTop:-3,marginBottom:4}}>{errs.dip}</div>}
          </Field>
        </div>
      </div>
      <Field label="Field Notes">
        <textarea value={f.notes||""} onChange={function(e){upd("notes",e.target.value);}} rows={2} placeholder="Additional observations…" style={Object.assign({},INP,{resize:"none"})}/>
      </Field>
      <div style={{fontSize:9,color:"#555",fontFamily:"monospace",marginBottom:8}}>
        {f.lat&&f.lon?toDMS(f.lat,true)+" "+toDMS(f.lon,false):""}
      </div>
  ...
  return(
    <div style={{background:"#12122e",border:"1px solid #2a1a5a",borderRadius:8,padding:10}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
        <div style={{width:16,height:16,background:ROCK_COLORS[f.rock]||"#ccc",borderRadius:3,border:"1px solid #fff",flexShrink:0}}/>
        <span style={{fontSize:9,color:"#888"}}>{f.rock}</span>
      </div>
      <Field label="Formation Name">
        <input value={f.formation||""} onChange={function(e){upd("formation",e.target.value);}} placeholder="e.g. Asu River Group" style={INP}/>
      </Field>
      <Field label="Geological Period">
        <select value={f.period||"Unknown"} onChange={function(e){upd("period",e.target.value);}} style={SEL}>
          {GEO_PERIODS.map(function(p){return <option key={p}>{p}</option>;})}
        </select>
      </Field>
      <Field label="Contact Type">
        <select value={f.contact||"Unknown"} onChange={function(e){upd("contact",e.target.value);}} style={SEL}>
          {CONTACT_TYPES.map(function(c){return <option key={c}>{c}</option>;})}
        </select>
      </Field>
...
function RoadEditPanel({road,onSave,onDelete,onDeselect}){
  ...
  return(
    <div style={{background:"#12122e",border:"1px solid #5a2a00",borderRadius:8,padding:10}}>
      ...
      <Field label="Road Name (optional)">
        <input value={f.name||""} onChange={function(e){upd("name",e.target.value);}} placeholder="e.g. Aba-Umuahia Rd" style={INP}/>
      </Field>
      <Field label="Surface Type">
        <select value={f.surface||"Paved"} onChange={function(e){upd("surface",e.target.value);}} style={SEL}>
          {ROAD_SURFACES.map(function(s){return <option key={s}>{s}</option>;})}
        </select>
      </Field>
      <div style={{fontSize:9,color:"#555",marginBottom:8}}>{(f.points||[]).length} nodes · {road.type==="major"?"Major":"Minor"} road</div>
      <div style={{display:"flex",gap:4}}>
        <button onClick={function(){onSave(f);}} style={{flex:1,background:"#1a4a2a",color:"#27ae60",border:"1px solid #27ae60",borderRadius:4,padding:"6px",fontSize:10,cursor:"pointer",fontWeight:"bold"}}>✓ Save</button>
        <button onClick={onDelete} style={{background:"#3a0a0a",color:"#e74c3c",border:"1px solid #e74c3c",borderRadius:4,padding:"6px 8px",fontSize:10,cursor:"pointer"}}>🗑</button>
        <button onClick={onDeselect} style={{background:"#1a1a2a",color:"#888",border:"1px solid #3a3a6a",borderRadius:4,padding:"6px 8px",fontSize:10,cursor:"pointer"}}>✕</button>
...
function RiverEditPanel({river,onSave,onDelete,onDeselect}){
  ...
  return(
    <div style={{background:"#12122e",border:"1px solid #003a5a",borderRadius:8,padding:10}}>
      ...
      <Field label="River Name">
        <input value={f.name||""} onChange={function(e){upd("name",e.target.value);}} placeholder="e.g. Cross River" style={INP}/>
      </Field>
      <Field label="Flow Direction">
        <select value={f.flow||"Unknown"} onChange={function(e){upd("flow",e.target.value);}} style={SEL}>
          {RIVER_FLOW.map(function(d){return <option key={d}>{d}</option>;})}
        </select>
      </Field>
      <div style={{fontSize:9,color:"#555",marginBottom:8}}>{(f.points||[]).length} nodes</div>
      <div style={{display:"flex",gap:4}}>
  ...
  return(
    <div style={{background:"#12122e",border:"1px solid #2a2a5a",borderRadius:8,padding:10,marginBottom:8}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
        ...
        <span style={{fontSize:16,fontWeight:"bold",color:color}}>{score}%</span>
      </div>
      ...
      <div style={{fontSize:9,color:"#444",marginTop:5}}>
        {score<50?"Fill in feature attributes to improve quality":score<80?"Good progress — add formation names and field descriptions":"Map data is well documented"}
      </div>
    </div>
  );
}
...
// ── renderMap (unchanged from original, passed-through) ────────────
...
function drawAll(){
  ...
  roads.forEach(function(road){
    ...
    if(road.type==="major"){drawSmooth(ctx,pts);ctx.strokeStyle="#c0392b";ctx.lineWidth=3;ctx.stroke();drawSmooth(ctx,pts);ctx.strokeStyle="#e07030";ctx.lineWidth=1.5;ctx.stroke();}
    else{drawSmooth(ctx,pts);ctx.strokeStyle="#888";ctx.lineWidth=1.2;ctx.stroke();}
    if(road.name&&road.name.trim()){var mid=road.points[Math.floor(road.points.length/2)],mp=ll2px(mid.lat,mid.lon);ctx.fillStyle="#333";ctx.font="7px Times New Roman";ctx.fillText(road.name,mp.x+2,mp.y-3);}
  });
  rivers.forEach(function(river){
    ...
    drawSmooth(ctx,pts);ctx.strokeStyle="#2471a3";ctx.lineWidth=1.8;ctx.stroke();
    if(river.name&&river.name.trim()){var mid=river.points[Math.floor(river.points.length/2)],mp=ll2px(mid.lat,mid.lon);ctx.fillStyle="#2471a3";ctx.font="italic 7px Times New Roman";ctx.fillText(river.name,mp.x+2,mp.y-3);}
  });
...
export default function GeoMappingSystem(){
  ...
  // ── Draw-mode entry forms (for new features before placing) ──────
  ...
  useEffect(function(){
    ...
    geoZones.forEach(function(z2,gi){
      ...
      ctx.beginPath();
      z2.points.forEach(function(pt,i){var pp=p(pt.lat,pt.lon);if(i===0)ctx.moveTo(pp.x,pp.y);else ctx.lineTo(pp.x,pp.y);});
      ...
      ctx.strokeStyle=isSel?"#f0c040":isActive?"#27ae60":ROCK_COLORS[z2.rock]||"#aaa";
      ctx.lineWidth=isSel?3:isActive?2:2;
      if(isActive)ctx.setLineDash([5,3]);
      ctx.stroke();ctx.setLineDash([]);
      if(isActive){
        z2.points.forEach(function(pt,i){
          var pp=p(pt.lat,pt.lon);
          ctx.fillStyle=i===0?"#27ae60":"#fff";
          ctx.beginPath();ctx.arc(pp.x,pp.y,i===0?6:4,0,Math.PI*2);ctx.fill();
          if(i===0){ctx.strokeStyle="#27ae60";ctx.lineWidth=2;ctx.beginPath();ctx.arc(pp.x,pp.y,10,0,Math.PI*2);ctx.stroke();}
        });
        if(mousePos){var last2=z2.points[z2.points.length-1],lp=p(last2.lat,last2.lon),mp=p(mousePos.lat,mousePos.lon);ctx.beginPath();ctx.moveTo(lp.x,lp.y);ctx.lineTo(mp.x,mp.y);ctx.strokeStyle="#27ae60";ctx.lineWidth=1;ctx.setLineDash([3,3]);ctx.stroke();ctx.setLineDash([]);}
      }
      if(!isActive&&z2.points.length>2){
        ...
        var cp=p(cy2,cx2);ctx.fillStyle="#fff";ctx.font="bold 9px sans-serif";ctx.textAlign="center";
        ctx.fillText(z2.formation&&z2.formation.trim()?z2.formation:z2.rock,cp.x,cp.y);ctx.textAlign="left";
      }
    });
    ...
    roads.forEach(function(road,ri){
      ...
      if(road.type==="major"){
        ...
        ctx.strokeStyle=isSel?"#f0c040":"#c0392b";ctx.lineWidth=4;ctx.stroke();
        if(pts.length>=2)drawSmooth(ctx,pts);
        ctx.strokeStyle=isSel?"#ffe080":"#e07030";ctx.lineWidth=2;ctx.stroke();
      } else {
        if(pts.length>=2)drawSmooth(ctx,pts);else{ctx.beginPath();ctx.moveTo(pts[0].x,pts[0].y);}
        ctx.strokeStyle=isSel?"#f0c040":"#888";ctx.lineWidth=1.8;ctx.stroke();
      }
      ctx.shadowBlur=0;
      ...
      if(isActive){
        pts.forEach(function(pp){
          ctx.fillStyle=road.type==="major"?"#e07030":"#aaa";
          ctx.beginPath();ctx.arc(pp.x,pp.y,4,0,Math.PI*2);ctx.fill();
          ctx.strokeStyle="#fff";ctx.lineWidth=1;ctx.beginPath();ctx.arc(pp.x,pp.y,4,0,Math.PI*2);ctx.stroke();
        });
    ...
    rivers.forEach(function(river,ri){
      ...
      if(isActive){
        pts.forEach(function(pp){
          ctx.fillStyle="#2980d9";ctx.beginPath();ctx.arc(pp.x,pp.y,4,0,Math.PI*2);ctx.fill();
          ctx.strokeStyle="#fff";ctx.lineWidth=1;ctx.beginPath();ctx.arc(pp.x,pp.y,4,0,Math.PI*2);ctx.stroke();
        });
        if(mousePos&&pts.length>0){
          var last=pts[pts.length-1],mp=p(mousePos.lat,mousePos.lon);
    ...
    towns.forEach(function(town,ti){
      ...
      var isSel=selectedFeature&&selectedFeature.type==="town"&&selectedFeature.id===ti;
      ...
      ctx.shadowBlur=0;
      ctx.fillStyle=isSel?"#f0c040":"#000";ctx.font="bold 9px sans-serif";ctx.fillText(town.name||"Town",pp.x+6,pp.y-3);
    });

    samples.forEach(function(s,si){
      var pp=p(s.lat,s.lon);
      var isSel=selectedFeature&&selectedFeature.type==="sample"&&selectedFeature.id===si;
      if(isSel){ctx.shadowColor="#f0c040";ctx.shadowBlur=12;}
      ctx.fillStyle=isSel?"#f0c040":"#c0392b";
      var sz=isSel?11:9;
      ...
      ctx.shadowBlur=0;
      ctx.fillStyle=isSel?"#f0c040":"#c0392b";ctx.font="6px sans-serif";ctx.fillText(s.id,pp.x+sz+1,pp.y+2);
      // Strike/dip symbol if data present
      if(s.strike&&s.dip){
        ...
        ctx.fillStyle="#27ae60";ctx.font="6px sans-serif";ctx.fillText(s.dip+"°",sl*0.6*Math.sin(da)+1,sl*0.6*Math.cos(da));
        ctx.restore();
      }
    });
    ...
    if(previewPin){
      ...
      ctx.save();ctx.strokeStyle="#f0c040";ctx.lineWidth=2;
    ...
    var ax=W-36,ay=36;
    ctx.save();ctx.fillStyle="#fff";ctx.beginPath();ctx.arc(ax,ay,18,0,Math.PI*2);ctx.fill();
    ...
    ctx.fillStyle="#c0392b";ctx.font="bold 10px sans-serif";ctx.textAlign="center";ctx.fillText("N",ax,ay-17);ctx.textAlign="left";ctx.restore();
    ...
    ctx.save();ctx.fillStyle="rgba(255,255,255,0.88)";ctx.fillRect(sx-3,sy-12,bp+6,18);
    ctx.fillStyle="#222";ctx.fillRect(sx,sy-6,bp/2,7);ctx.fillStyle="#999";ctx.fillRect(sx+bp/2,sy-6,bp/2,7);
    ctx.strokeStyle="#222";ctx.lineWidth=1;ctx.strokeRect(sx,sy-6,bp,7);
    ctx.fillStyle="#222";ctx.font="8px sans-serif";ctx.fillText("0",sx,sy-8);ctx.fillText(bm>=1000?bm/1000+"km":bm+"m",sx+bp-6,sy-8);ctx.restore();
  ...
  function handleClick(e){
    ...
    if(mode==="town"){
      ...
      setTowns(function(t){return t.concat([{lat:ll.lat,lon:ll.lon,name:name,townType:townForm.townType||"Settlement"}]);});
      setTownForm({name:"",townType:"Settlement"});
    }
    else if(mode==="road-major"||mode==="road-minor"){
      if(activeRoadIdx===null){
        var newRoad={type:mode==="road-major"?"major":"minor",points:[ll],name:"",surface:"Paved"};
        setRoads(function(prev){setActiveRoadIdx(prev.length);return prev.concat([newRoad]);});
      } else {
        setRoads(function(prev){return prev.map(function(r,i){if(i===activeRoadIdx)return Object.assign({},r,{points:r.points.concat([ll])});return r;});});
      }
    }
    else if(mode==="river"){
      if(activeRiverIdx===null){
        var newRiver={points:[ll],name:"",flow:"Unknown"};
        setRivers(function(prev){setActiveRiverIdx(prev.length);return prev.concat([newRiver]);});
      } else {
        setRivers(function(prev){return prev.map(function(r,i){if(i===activeRiverIdx)return Object.assign({},r,{points:r.points.concat([ll])});return r;});});
      }
    }
    else if(mode==="sample"){
      // Validate before placing
      var errs={};
      var idErr=validateSampleId(sampleForm.id,samples,-1);
      if(idErr)errs.id=idErr;
      var sErr=validateStrike(sampleForm.strike);
      if(sErr)errs.strike=sErr;
      var dErr=validateDip(sampleForm.dip);
      if(dErr)errs.dip=dErr;
      ...
      setSampleForm(function(f){return Object.assign({},f,{id:"",description:"",strike:"",dip:"",notes:""});});
      setSampleFormErrs({});
    }
    else if(mode==="geology"){
      if(activeGeoIdx===null){
        var newGeo={rock:geoRock,formation:"",period:"Unknown",contact:"Unknown",points:[ll]};
        setGeoZones(function(prev){setActiveGeoIdx(prev.length);return prev.concat([newGeo]);});
      } else {
        var firstPt=geoZones[activeGeoIdx].points[0];
        var fp2=ll2px(firstPt.lat,firstPt.lon,center.lat,center.lon,zoom,W,H);
        var cp2=ll2px(ll.lat,ll.lon,center.lat,center.lon,zoom,W,H);
        if(geoZones[activeGeoIdx].points.length>2&&dist(fp2,cp2)<16){
          setActiveGeoIdx(null);return;
        }
  ...
  // ── Save edited feature back to state ───────────────────────────
  ...
  function onMouseMove(e){
    ...
    if(mode==="pan"&&dragRef.current){
      var d=dragRef.current,ws=TILE_SIZE*Math.pow(2,zoom),r=canvasRef.current.getBoundingClientRect();
  ...
  function finishGeo(){
    if(activeGeoIdx!==null&&geoZones[activeGeoIdx]&&geoZones[activeGeoIdx].points.length<3){
      setGeoZones(function(prev){return prev.filter(function(_,i){return i!==activeGeoIdx;});});
    }
    setActiveGeoIdx(null);
  }

  function undoLastNode(){
    if((mode==="road-major"||mode==="road-minor")&&activeRoadIdx!==null){
      setRoads(function(prev){return prev.map(function(r,i){if(i===activeRoadIdx){if(r.points.length<=1){setActiveRoadIdx(null);return null;}return Object.assign({},r,{points:r.points.slice(0,-1)});}return r;}).filter(Boolean);});
    }
    else if(mode==="river"&&activeRiverIdx!==null){
      setRivers(function(prev){return prev.map(function(r,i){if(i===activeRiverIdx){if(r.points.length<=1){setActiveRiverIdx(null);return null;}return Object.assign({},r,{points:r.points.slice(0,-1)});}return r;}).filter(Boolean);});
    }
    else if(mode==="geology"&&activeGeoIdx!==null){
      setGeoZones(function(prev){return prev.map(function(z,i){if(i===activeGeoIdx){if(z.points.length<=1){setActiveGeoIdx(null);return null;}return Object.assign({},z,{points:z.points.slice(0,-1)});}return z;}).filter(Boolean);});
    }
  }

  function clearAll(){
    setTowns([]);setRoads([]);setRivers([]);setSamples([]);setGeoZones([]);
    setActiveRoadIdx(null);setActiveRiverIdx(null);setActiveGeoIdx(null);
    setPreviewPin(null);setSelectedFeature(null);
  }
  ...
  function placeFromCoord(ll){
    ...
    if(mode==="town"){var name=townForm.name||"Town "+(towns.length+1);setTowns(function(t){return t.concat([{lat:ll.lat,lon:ll.lon,name:name,townType:townForm.townType||"Settlement"}]);});setTownForm({name:"",townType:"Settlement"});}
      ...
      else{setGeoZones(function(prev){return prev.map(function(z,i){if(i===activeGeoIdx)return Object.assign({},z,{points:z.points.concat([ll])});return z;});});}
    }
  ...
  // Build validation warnings list
  ...
  samples.forEach(function(s,i){
    if(s.strike){var n=parseFloat(s.strike);if(isNaN(n)||n<0||n>360)validationErrors.push("Sample "+(i+1)+" ("+s.id+"): Strike out of range");}
    if(s.dip){var n=parseFloat(s.dip);if(isNaN(n)||n<0||n>90)validationErrors.push("Sample "+(i+1)+" ("+s.id+"): Dip out of range");}
  });
  geoZones.forEach(function(z,i){if(z.points.length>0&&z.points.length<3)validationErrors.push("Geology zone "+(i+1)+" ("+z.rock+"): needs at least 3 nodes to form a polygon");});
  ...
  return(
    <div style={{background:"#0d0d1f",height:"100vh",fontFamily:"sans-serif",color:"#eee",display:"flex",flexDirection:"column",overflow:"hidden"}}>
      ...
      <div style={{background:"#12122e",borderBottom:"1px solid #2a2a5a",padding:"7px 14px",display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <div>
            ...
            <div style={{fontSize:9,color:"#555"}}>Nigeria Geological Survey — Sequence 5A</div>
          </div>
        </div>
        <div style={{display:"flex",gap:6,alignItems:"center"}}>
          <input value={projectName} onChange={function(e){setProjectName(e.target.value);}} placeholder="Study area name…"
      ...
      <div style={{display:"flex",flex:1,overflow:"hidden"}}>
        ...
        <div ref={containerRef} style={{flex:1,position:"relative",overflow:"hidden"}}>
          ...
          <div style={{position:"absolute",bottom:0,left:0,right:0,background:"rgba(0,0,0,0.85)",borderTop:"1px solid #2a2a5a",padding:"4px 12px",display:"flex",gap:16,alignItems:"center",pointerEvents:"none"}}>
            ...
            {cursorLL?(
              <>
                <span style={{fontSize:11,color:"#f0c040",fontFamily:"monospace",fontWeight:"bold"}}>{toDMS(cursorLL.lat,true)}</span>
                <span style={{fontSize:11,color:"#f0c040",fontFamily:"monospace",fontWeight:"bold"}}>{toDMS(cursorLL.lon,false)}</span>
                <span style={{fontSize:10,color:"#555",fontFamily:"monospace"}}>({cursorLL.lat.toFixed(5)}, {cursorLL.lon.toFixed(5)})</span>
              </>
            ):(
          ...
          {(mode==="road-major"||mode==="road-minor")&&(
            <div style={{position:"absolute",bottom:28,left:"50%",transform:"translateX(-50%)",background:"rgba(0,0,0,0.75)",color:"#fff",borderRadius:8,padding:"6px 14px",fontSize:11,display:"flex",gap:8}}>
              <span>{activeRoad?activeRoad.points.length+" nodes — click to add more":"Click to start road"}</span>
          ...
          {mode==="river"&&(
            <div style={{position:"absolute",bottom:28,left:"50%",transform:"translateX(-50%)",background:"rgba(0,0,0,0.75)",color:"#fff",borderRadius:8,padding:"6px 14px",fontSize:11,display:"flex",gap:8}}>
              <span>{activeRiver?activeRiver.points.length+" nodes — click to add more":"Click to start river"}</span>
          ...
          {mode==="geology"&&(
            <div style={{position:"absolute",bottom:28,left:"50%",transform:"translateX(-50%)",background:"rgba(0,0,0,0.75)",color:"#fff",borderRadius:8,padding:"6px 14px",fontSize:11,display:"flex",gap:8,alignItems:"center"}}>
              {!activeGeo?"Click to start polygon":activeGeo.points.length+" nodes · click green circle to close"}
          ...
          {mode==="select"&&(
            <div style={{position:"absolute",bottom:28,left:"50%",transform:"translateX(-50%)",background:"rgba(0,20,0,0.85)",color:"#27ae60",borderRadius:8,padding:"6px 14px",fontSize:11}}>
              Click any feature to select &amp; edit attributes
            </div>
          )}
        </div>
        ...
        <div style={{width:250,background:"#0a0a1e",borderLeft:"1px solid #2a2a5a",display:"flex",flexDirection:"column",flexShrink:0}}>
          <div style={{display:"flex",borderBottom:"1px solid #2a2a5a"}}>
            {["draw","data","output"].map(function(t){return(
              <button key={t} onClick={function(){setTab(t);}}
                ...
                {t==="draw"?"✏️ Draw":t==="data"?"📊 Data":"🗺 Output"}
              </button>
            );})}
          </div>
          <div style={{flex:1,overflowY:"auto",padding:12}}>
            ...
            {tab==="draw"&&(
              <div style={{display:"flex",flexDirection:"column",gap:10}}>
                ...
                {/* Select mode — show edit panels */}
                {mode==="select"&&(
                  selectedFeature?(
                    selectedFeature.type==="town"?
                      <TownEditPanel town={towns[selectedFeature.id]} onSave={saveSelected} onDelete={deleteSelected} onDeselect={function(){setSelectedFeature(null);}}/>
                    :selectedFeature.type==="sample"?
                      <SampleEditPanel sample={samples[selectedFeature.id]} allSamples={samples} editingIdx={selectedFeature.id} onSave={saveSelected} onDelete={deleteSelected} onDeselect={function(){setSelectedFeature(null);}}/>
                    :selectedFeature.type==="geology"?
                      <GeoEditPanel zone={geoZones[selectedFeature.id]} onSave={saveSelected} onDelete={deleteSelected} onDeselect={function(){setSelectedFeature(null);}}/>
                    :selectedFeature.type==="road"?
                      <RoadEditPanel road={roads[selectedFeature.id]} onSave={saveSelected} onDelete={deleteSelected} onDeselect={function(){setSelectedFeature(null);}}/>
                    :selectedFeature.type==="river"?
                      <RiverEditPanel river={rivers[selectedFeature.id]} onSave={saveSelected} onDelete={deleteSelected} onDeselect={function(){setSelectedFeature(null);}}/>
                    :null
                  ):(
                    <div style={{background:"#12122e",border:"1px solid #1a3a1a",borderRadius:8,padding:12,textAlign:"center"}}>
                      <div style={{fontSize:20,marginBottom:6}}>👆</div>
                      <div style={{fontSize:11,color:"#27ae60",fontWeight:"bold",marginBottom:4}}>SELECT MODE</div>
                      <div style={{fontSize:10,color:"#555",lineHeight:1.7}}>Click any feature on the map to select it and edit its attributes.</div>
                    </div>
                  )
                )}
                ...
                {mode==="town"&&(
                  <div style={{background:"#12122e",border:"1px solid #1a3a5a",borderRadius:8,padding:10}}>
                    ...
                    <Field label="Town Name">
                      <input value={townForm.name} onChange={function(e){setTownForm(function(f){return Object.assign({},f,{name:e.target.value});});}} placeholder="e.g. Ananamong" style={INP}/>
                    </Field>
                    <Field label="Settlement Type">
                      <select value={townForm.townType||"Settlement"} onChange={function(e){setTownForm(function(f){return Object.assign({},f,{townType:e.target.value});});}} style={SEL}>
                        {TOWN_TYPES.map(function(t){return <option key={t}>{t}</option>;})}
                      </select>
                    </Field>
                    <div style={{fontSize:10,color:"#555",marginBottom:4}}>Click map to place</div>
                    <CoordInput label="Place by Coordinates" onPlace={placeFromCoord}/>
                  </div>
                )}
                ...
                {(mode==="road-major"||mode==="road-minor")&&(
                  <div style={{background:"#12122e",border:"1px solid #5a2a00",borderRadius:8,padding:10}}>
                    ...
                    <div style={{fontSize:10,color:"#888",lineHeight:1.6,marginBottom:6}}>Click as many nodes as you need · curves through all points · finish when done</div>
                    {activeRoad&&<div style={{marginBottom:6,fontSize:10,color:"#f0c040"}}>{activeRoad.points.length} nodes placed</div>}
                ...
                {mode==="river"&&(
                  <div style={{background:"#12122e",border:"1px solid #003a5a",borderRadius:8,padding:10}}>
                    ...
                    <div style={{fontSize:10,color:"#888",lineHeight:1.6,marginBottom:6}}>Click as many nodes as you need · curves through all points · finish when done</div>
                    {activeRiver&&<div style={{marginBottom:6,fontSize:10,color:"#f0c040"}}>{activeRiver.points.length} nodes placed</div>}
                    <CoordInput label="Add Node by Coordinates" onPlace={placeFromCoord}/>
                  </div>
                )}
                ...
                {mode==="sample"&&(
                  <div style={{background:"#12122e",border:"1px solid #5a1a1a",borderRadius:8,padding:10}}>
                    ...
                    <Field label="Sample ID *">
                      <input value={sampleForm.id} onChange={function(e){setSampleForm(function(f){return Object.assign({},f,{id:e.target.value});});setSampleFormErrs(function(e2){return Object.assign({},e2,{id:null});});}} placeholder="e.g. UU/GS/GLG/25/57" style={INP}/>
                      {sampleFormErrs.id&&<div style={{fontSize:9,color:"#e74c3c",marginTop:-3,marginBottom:4}}>{sampleFormErrs.id}</div>}
                    </Field>
                    <Field label="Rock Type">
                      <select value={sampleForm.rock} onChange={function(e){setSampleForm(function(f){return Object.assign({},f,{rock:e.target.value});});}} style={SEL}>
                        {ROCK_TYPES.map(function(r){return <option key={r}>{r}</option>;})}
                      </select>
                    </Field>
                    <Field label="Field Description">
                      <input value={sampleForm.description||""} onChange={function(e){setSampleForm(function(f){return Object.assign({},f,{description:e.target.value});});}} placeholder="e.g. dark grey, finely laminated" style={INP}/>
                    </Field>
                    <div style={{display:"flex",gap:6}}>
                      <div style={{flex:1}}>
                        <Field label="Strike (0–360°)">
                          <input value={sampleForm.strike||""} onChange={function(e){setSampleForm(function(f){return Object.assign({},f,{strike:e.target.value});});setSampleFormErrs(function(e2){return Object.assign({},e2,{strike:null});});}} placeholder="e.g. 045" style={INP}/>
                          {sampleFormErrs.strike&&<div style={{fontSize:9,color:"#e74c3c",marginTop:-3,marginBottom:4}}>{sampleFormErrs.strike}</div>}
                        </Field>
                      </div>
                      <div style={{flex:1}}>
                        <Field label="Dip (0–90°)">
                          <input value={sampleForm.dip||""} onChange={function(e){setSampleForm(function(f){return Object.assign({},f,{dip:e.target.value});});setSampleFormErrs(function(e2){return Object.assign({},e2,{dip:null});});}} placeholder="e.g. 32" style={INP}/>
                          {sampleFormErrs.dip&&<div style={{fontSize:9,color:"#e74c3c",marginTop:-3,marginBottom:4}}>{sampleFormErrs.dip}</div>}
                        </Field>
                      </div>
                    </div>
                    <Field label="Field Notes">
                      <textarea value={sampleForm.notes||""} onChange={function(e){setSampleForm(function(f){return Object.assign({},f,{notes:e.target.value});});}} rows={2} placeholder="Additional observations…" style={Object.assign({},INP,{resize:"none"})}/>
                    </Field>
                    <div style={{fontSize:10,color:"#555",marginBottom:4}}>Click map or enter coordinates to place</div>
                    <CoordInput label="Place by Coordinates" onPlace={placeFromCoord}/>
                  </div>
                )}
                ...
                {mode==="geology"&&(
                  <div style={{background:"#12122e",border:"1px solid #2a1a5a",borderRadius:8,padding:10}}>
                    ...
                    <Field label="Rock Type">
                      <select value={geoRock} onChange={function(e){setGeoRock(e.target.value);}} style={SEL}>
                        {ROCK_TYPES.map(function(r){return <option key={r}>{r}</option>;})}
                      </select>
                    </Field>
                    ...
                    <div style={{fontSize:10,color:"#888",lineHeight:1.6,marginBottom:6}}>Click nodes · green circle to close · min 3 points<br/>After placing, switch to 👆 Select to add formation name</div>
                ...
                {mode==="pan"&&(
                  <div style={{fontSize:11,color:"#555",textAlign:"center",padding:20,lineHeight:1.8}}>Select a drawing tool above to begin.<br/><br/>Use <span style={{color:"#27ae60"}}>👆 Select</span> to click any placed feature and edit its attributes.</div>
                )}
            ...
            {tab==="data"&&(
              <div style={{display:"flex",flexDirection:"column",gap:8}}>
                ...
                {[
                  ...
                  <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",background:"#12122e",borderRadius:6,padding:"7px 10px",borderLeft:"3px solid "+item.color}}>
                ...
                {samples.length>0&&(
                  <div style={{background:"#12122e",border:"1px solid #2a2a5a",borderRadius:8,padding:10,marginTop:4}}>
                    <div style={{fontSize:11,color:"#f0c040",fontWeight:"bold",marginBottom:6}}>SAMPLES</div>
                    {samples.map(function(s,i){
                      var sc=scoreFeature("sample",s);
                      var pct=Math.round(sc.score/sc.max*100);
                      return(
                        <div key={i} style={{borderBottom:"1px solid #1a1a3a",padding:"5px 0",cursor:"pointer"}}
                          onClick={function(){setSelectedFeature({type:"sample",id:i});setTab("draw");setMode("select");}}>
                          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                            <span style={{fontSize:10,color:"#ffaaaa",fontWeight:"bold"}}>{s.id||"(unnamed)"}</span>
                            <span style={{fontSize:9,color:pct===100?"#27ae60":pct>=60?"#f0c040":"#e74c3c"}}>{pct}%</span>
                          </div>
                          <div style={{display:"flex",justifyContent:"space-between"}}>
                            <span style={{fontSize:9,color:"#888"}}>{s.rock}{s.description?` · ${s.description.slice(0,20)}…`:""}</span>
                            {s.strike&&s.dip&&<span style={{fontSize:9,color:"#7ab"}}>{s.strike}/{s.dip}°</span>}
                          </div>
                          <div style={{fontSize:8,color:"#444",fontFamily:"monospace"}}>{s.lat.toFixed(4)}, {s.lon.toFixed(4)}</div>
                        </div>
                      );
                    })}
                  </div>
                ...
                {geoZones.length>0&&(
                  <div style={{background:"#12122e",border:"1px solid #2a2a5a",borderRadius:8,padding:10}}>
                    <div style={{fontSize:11,color:"#f0c040",fontWeight:"bold",marginBottom:6}}>GEOLOGY ZONES</div>
                    {geoZones.map(function(z,i){
                      var sc=scoreFeature("geology",z);
                      var pct=Math.round(sc.score/sc.max*100);
                      return(
                        <div key={i} style={{borderBottom:"1px solid #1a1a3a",padding:"5px 0",cursor:"pointer"}}
                          onClick={function(){setSelectedFeature({type:"geology",id:i});setTab("draw");setMode("select");}}>
                          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:6}}>
                            <div style={{width:10,height:10,background:ROCK_COLORS[z.rock]||"#ccc",borderRadius:2,flexShrink:0}}/>
                            <span style={{fontSize:10,color:"#cca0ff",fontWeight:"bold",flex:1}}>{z.formation||z.rock}</span>
                            <span style={{fontSize:9,color:pct===100?"#27ae60":pct>=60?"#f0c040":"#e74c3c"}}>{pct}%</span>
                          </div>
                          <div style={{fontSize:9,color:"#666",marginLeft:16}}>{z.rock} · {z.period} · {z.points.length} nodes</div>
                        </div>
                      );
                    })}
                  </div>
                ...
                {towns.length>0&&(
                  <div style={{background:"#12122e",border:"1px solid #2a2a5a",borderRadius:8,padding:10}}>
                    <div style={{fontSize:11,color:"#f0c040",fontWeight:"bold",marginBottom:6}}>TOWNS</div>
                    {towns.map(function(t,i){
                      return(
                        <div key={i} style={{borderBottom:"1px solid #1a1a3a",padding:"4px 0",cursor:"pointer",display:"flex",justifyContent:"space-between"}}
                          onClick={function(){setSelectedFeature({type:"town",id:i});setTab("draw");setMode("select");}}>
                          <span style={{fontSize:10,color:"#aaddff"}}>{t.name||"(unnamed)"}</span>
                          <span style={{fontSize:9,color:"#555"}}>{t.townType||"Settlement"}</span>
                        </div>
                      );
                    })}
                  </div>
            ...
            {tab==="output"&&(
              <div style={{display:"flex",flexDirection:"column",gap:10}}>
                ...
                <div style={{background:"#12122e",border:"1px solid #2a2a5a",borderRadius:8,padding:10}}>
                  ...
                  {DPI_OPTIONS.map(function(opt){var active=exportDPI===opt.dpi;return(
                    <div key={opt.dpi} onClick={function(){setExportDPI(opt.dpi);}}
                      style={{display:"flex",justifyContent:"space-between",alignItems:"center",background:active?"#1a3a1a":"#0f0f2a",border:"1px solid "+(active?"#27ae60":"#2a2a5a"),borderRadius:5,padding:"6px 8px",marginBottom:4,cursor:"pointer"}}>
                ...
                <div style={{background:"#0f0f1e",border:"1px dashed #2a2a4a",borderRadius:8,padding:10}}>
                  <div style={{fontSize:10,color:"#444",fontWeight:"bold",marginBottom:4}}>NEXT: SEQUENCE 5B</div>
      ...
      <div style={{background:"#0a0a1e",borderTop:"1px solid #2a2a5a",padding:"4px 14px",display:"flex",justifyContent:"space-between",flexShrink:0}}>
        <span style={{fontSize:9,color:"#333"}}>Geo Mapping System v0.5A — Data Forms + Validation</span>

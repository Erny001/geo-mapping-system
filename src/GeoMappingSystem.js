...
<script>
var W=${W}, H=${H};
var MARGIN={top:${MARGIN.top},left:${MARGIN.left},right:${MARGIN.right},bottom:${MARGIN.bottom}};
var MAP_W=${MAP_W}, MAP_H=${MAP_H};
var center=${JSON.stringify(center)};
var zoom=${zoom};
var towns=${JSON.stringify(towns)};
var roads=${JSON.stringify(roads)};
var rivers=${JSON.stringify(rivers)};
var samples=${JSON.stringify(samples)};
var geoZones=${JSON.stringify(geoZones)};
...
function drawAll(){
  ...
  if(mapType==="map3"){
    geoZones.forEach(function(z){
      ...
      ctx.fillStyle="#000"; ctx.font="bold 8px Times New Roman"; ctx.textAlign="center";
      ctx.fillText(z.rock,cp.x,cp.y); ctx.textAlign="left";
    });
  }
  ...
  rivers.forEach(function(river){
    ...
    ctx.beginPath();
    river.points.forEach(function(pt,i){var pp=ll2px(pt.lat,pt.lon);if(i===0)ctx.moveTo(pp.x,pp.y);else ctx.lineTo(pp.x,pp.y);});
  ...
  for(var lo=Math.ceil(tl.lon/step)*step;lo<=br.lon;lo+=step){
    ...
    ctx.beginPath();ctx.moveTo(ppx,MARGIN.top);ctx.lineTo(ppx,MARGIN.top+MAP_H);ctx.stroke();
    ctx.fillText(toDMS(lo,false),ppx-14,MARGIN.top+MAP_H+12);
    ctx.fillText(toDMS(lo,false),ppx-14,MARGIN.top-4);
  }
...
export default function GeoMappingSystem() {
  ...
  useEffect(function(){
    ...
    if(activeGeo&&activeGeo.points.length>0){
      ctx.beginPath();
      activeGeo.points.forEach(function(pt,i){var pp=p(pt.lat,pt.lon);if(i===0)ctx.moveTo(pp.x,pp.y);else ctx.lineTo(pp.x,pp.y);});
      ...
      activeGeo.points.forEach(function(pt){var pp=p(pt.lat,pt.lon);ctx.fillStyle="#fff";ctx.beginPath();ctx.arc(pp.x,pp.y,4,0,Math.PI*2);ctx.fill();});
      if(activeGeo.points.length>2){var fp=p(activeGeo.points[0].lat,activeGeo.points[0].lon);ctx.strokeStyle="#27ae60";ctx.lineWidth=2;ctx.beginPath();ctx.arc(fp.x,fp.y,8,0,Math.PI*2);ctx.stroke();}
    }

    roads.forEach(function(road){
      if(road.points.length<1)return;
      ctx.beginPath();
      road.points.forEach(function(pt,i){var pp=p(pt.lat,pt.lon);if(i===0)ctx.moveTo(pp.x,pp.y);else ctx.lineTo(pp.x,pp.y);});
      if(road===activeRoad&&mousePos){var mp=p(mousePos.lat,mousePos.lon);ctx.lineTo(mp.x,mp.y);}
      ctx.strokeStyle=road.type==="major"?"#e07030":"#888";ctx.lineWidth=road.type==="major"?3:1.5;ctx.stroke();
      if(road===activeRoad){road.points.forEach(function(pt){var pp=p(pt.lat,pt.lon);ctx.fillStyle=road.type==="major"?"#e07030":"#888";ctx.beginPath();ctx.arc(pp.x,pp.y,3,0,Math.PI*2);ctx.fill();});}
    });

    rivers.forEach(function(river){
      if(river.points.length<1)return;
      ctx.beginPath();
      river.points.forEach(function(pt,i){var pp=p(pt.lat,pt.lon);if(i===0)ctx.moveTo(pp.x,pp.y);else ctx.lineTo(pp.x,pp.y);});
    ...
    towns.forEach(function(town){
      ...
      ctx.fillStyle="#000";ctx.beginPath();ctx.arc(pp.x,pp.y,4,0,Math.PI*2);ctx.fill();
      ctx.fillStyle="#fff";ctx.beginPath();ctx.arc(pp.x,pp.y,2.5,0,Math.PI*2);ctx.fill();
      ctx.fillStyle="#000";ctx.font="bold 9px sans-serif";ctx.fillText(town.name||"Town",pp.x+6,pp.y-3);
    });

    samples.forEach(function(s){
      var pp=p(s.lat,s.lon);
      ctx.fillStyle="#c0392b";
      ctx.beginPath();ctx.moveTo(pp.x,pp.y-9);ctx.lineTo(pp.x+6,pp.y+4);ctx.lineTo(pp.x-6,pp.y+4);ctx.closePath();ctx.fill();
      ctx.fillStyle="#c0392b";ctx.font="6px sans-serif";ctx.fillText(s.id,pp.x+7,pp.y+2);
    });
    ...
    ctx.font="bold 11px sans-serif";ctx.fillText("MODE: "+MODE_LABELS[mode],14,23);ctx.restore();

    ctx.save();
    ctx.fillStyle="rgba(255,255,255,0.7)";ctx.fillRect(W-185,H-15,185,15);
    ctx.fillStyle="#666";ctx.font="8px sans-serif";
    ctx.fillText("\u00a9 OpenStreetMap contributors",W-182,H-4);ctx.restore();
  ...
  function handleClick(e){
    ...
    if(mode==="town"){
      ...
      if(!activeRoad){
        ...
        setRoads(function(prev){return prev.map(function(r){if(r===activeRoad){return Object.assign({},r,{points:r.points.concat([ll])});}return r;});});
      ...
      if(!activeRiver){
        ...
        setRivers(function(prev){return prev.map(function(r){if(r===activeRiver){return Object.assign({},r,{points:r.points.concat([ll])});}return r;});});
      ...
      if(!activeGeo){
        ...
        if(activeGeo.points.length>2){
          ...
          if(dist(fp,cp)<16){setGeoZones(function(z){return z.concat([activeGeo]);});setActiveGeo(null);return;}
        }
  ...
  function undoLastNode(){
    if(mode==="road-major"||mode==="road-minor"){
      if(activeRoad&&activeRoad.points.length>1){setRoads(function(prev){return prev.map(function(r){if(r===activeRoad){var np=r.points.slice(0,-1);var nr=Object.assign({},r,{points:np});setActiveRoad(nr);return nr;}return r;});});}
    } else if(mode==="river"){
      if(activeRiver&&activeRiver.points.length>1){setRivers(function(prev){return prev.map(function(r){if(r===activeRiver){var np=r.points.slice(0,-1);var nr=Object.assign({},r,{points:np});setActiveRiver(nr);return nr;}return r;});});}
    } else if(mode==="geology"){
      if(activeGeo&&activeGeo.points.length>1){setActiveGeo(function(prev){return prev?Object.assign({},prev,{points:prev.points.slice(0,-1)}):null;});}
    }
  }
      ...
      <div style={{display:"flex",flex:1,overflow:"hidden"}}>
        ...
        <div style={{width:240,background:"#0a0a1e",borderLeft:"1px solid #2a2a5a",display:"flex",flexDirection:"column",flexShrink:0}}>
          <div style={{display:"flex",borderBottom:"1px solid #2a2a5a"}}>
            {["draw","data","output"].map(function(t){
              return(
              ...
              <div style={{display:"flex",flexDirection:"column",gap:10}}>
                ...
                {mode==="geology"&&(
                  <div style={{background:"#12122e",border:"1px solid #2a1a5a",borderRadius:8,padding:10}}>
                    ...
                    {activeGeo&&<div style={{marginTop:8,fontSize:10,color:"#f0c040"}}>{activeGeo.points.length} nodes placed</div>}
                  </div>
                )}
                {mode==="pan"&&(

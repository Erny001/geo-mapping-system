    ...
    <div style={{background:"#0d1a2a",border:"1px solid #2a4a6a",borderRadius:7,padding:9,marginTop:6}}>
      ...
      <div style={{display:"flex",gap:4,marginBottom:4}}>
        <div style={{flex:1}}>
          ...
          <input value={latStr} onChange={function(e){setLatStr(e.target.value);setErr("");}} onKeyDown={function(e){if(e.key==="Enter")handle();}} placeholder={ph.lat}
        ...
        <div style={{flex:1}}>
          ...
          <input value={lonStr} onChange={function(e){setLonStr(e.target.value);setErr("");}} onKeyDown={function(e){if(e.key==="Enter")handle();}} placeholder={ph.lon}
            style={{width:"100%",background:"#1e2e3e",color:"#fff",border:"1px solid #3a5a7a",borderRadius:4,padding:"5px 6px",fontSize:10,boxSizing:"border-box"}}/>
        </div>
      </div>
...
function exportGeoJSON(towns,roads,rivers,samples,geoZones,projectName){
  ...
  var geojson={type:"FeatureCollection",name:projectName||"geo_mapping_export",features:features};
...
function exportKML(towns,roads,rivers,samples,geoZones,projectName){
  ...
  L.push('<kml xmlns="http://www.opengis.net/kml/2.2"><Document><name>'+esc(projectName||"Geo Mapping Export")+'</name>');
...
function exportCSV(samples,projectName){
  var headers=["Sample_ID","Rock_Type","Description","Latitude","Longitude","Strike","Dip","Notes"];
...
export default function GeoMappingSystem(){
  ...
  // All features stored as plain arrays — single source of truth
  ...
  // Active drawing tracked by INDEX into the arrays, not by object reference
  // This is the key fix — we always read from the state arrays, never from a stale ref
  ...
  useEffect(function(){
    ...
    // Geology zones — always drawn from state array
    geoZones.forEach(function(z2,gi){
      ...
      if(!isActive&&z2.points.length>2){
        ...
        var cp=p(cy2,cx2);ctx.fillStyle="#fff";ctx.font="bold 9px sans-serif";ctx.textAlign="center";ctx.fillText(z2.rock,cp.x,cp.y);ctx.textAlign="left";
      }
    });

    // Roads — always drawn from state array using index
    roads.forEach(function(road,ri){
      ...
      ctx.shadowBlur=0;
      // Show nodes and preview segment while active
      if(isActive){
    ...
    // Rivers — always drawn from state array using index
  ...
  function handleClick(e){
    ...
    if(mode==="select"){
      ...
      for(var ti=0;ti<towns.length;ti++){var tp=ll2px(towns[ti].lat,towns[ti].lon,center.lat,center.lon,zoom,W,H);if(dist(cp,tp)<SNAP){setSelectedFeature({type:"town",id:ti});return;}}
      for(var si=0;si<samples.length;si++){var sp=ll2px(samples[si].lat,samples[si].lon,center.lat,center.lon,zoom,W,H);if(dist(cp,sp)<SNAP){setSelectedFeature({type:"sample",id:si});return;}}
      for(var ri=0;ri<roads.length;ri++){for(var rj=0;rj<roads[ri].points.length;rj++){var rp=ll2px(roads[ri].points[rj].lat,roads[ri].points[rj].lon,center.lat,center.lon,zoom,W,H);if(dist(cp,rp)<SNAP){setSelectedFeature({type:"road",id:ri});return;}}}
      for(var rvi=0;rvi<rivers.length;rvi++){for(var rvj=0;rvj<rivers[rvi].points.length;rvj++){var rvp=ll2px(rivers[rvi].points[rvj].lat,rivers[rvi].points[rvj].lon,center.lat,center.lon,zoom,W,H);if(dist(cp,rvp)<SNAP){setSelectedFeature({type:"river",id:rvi});return;}}}
      for(var gi=0;gi<geoZones.length;gi++){for(var gj=0;gj<geoZones[gi].points.length;gj++){var gp=ll2px(geoZones[gi].points[gj].lat,geoZones[gi].points[gj].lon,center.lat,center.lon,zoom,W,H);if(dist(cp,gp)<SNAP){setSelectedFeature({type:"geology",id:gi});return;}}}
    ...
    if(mode==="town"){
      ...
      if(activeRoadIdx===null){
        // Start new road — push it and track its index
        var newRoad={type:mode==="road-major"?"major":"minor",points:[ll]};
        setRoads(function(prev){
          setActiveRoadIdx(prev.length);
          return prev.concat([newRoad]);
        });
      } else {
        // Append point to existing road at activeRoadIdx
        setRoads(function(prev){
          return prev.map(function(r,i){
            if(i===activeRoadIdx)return {type:r.type,points:r.points.concat([ll])};
            return r;
          });
        });
      }
      ...
      if(activeRiverIdx===null){
        var newRiver={points:[ll]};
        setRivers(function(prev){
          setActiveRiverIdx(prev.length);
          return prev.concat([newRiver]);
        });
      } else {
        setRivers(function(prev){
          return prev.map(function(r,i){
            if(i===activeRiverIdx)return {points:r.points.concat([ll])};
            return r;
          });
        });
      }
      ...
      if(activeGeoIdx===null){
        var newGeo={rock:geoRock,points:[ll]};
        setGeoZones(function(prev){
          setActiveGeoIdx(prev.length);
          return prev.concat([newGeo]);
        });
      } else {
        ...
        setGeoZones(function(prev){
          return prev.map(function(z,i){
            if(i===activeGeoIdx)return {rock:z.rock,points:z.points.concat([ll])};
            return z;
          });
        });
      }
  ...
  function undoLastNode(){
    if((mode==="road-major"||mode==="road-minor")&&activeRoadIdx!==null){
      setRoads(function(prev){
        return prev.map(function(r,i){
          if(i===activeRoadIdx){
            if(r.points.length<=1){setActiveRoadIdx(null);return null;}
            return {type:r.type,points:r.points.slice(0,-1)};
          }
          return r;
        }).filter(Boolean);
      });
      ...
      setRivers(function(prev){
        return prev.map(function(r,i){
          if(i===activeRiverIdx){
            if(r.points.length<=1){setActiveRiverIdx(null);return null;}
            return {points:r.points.slice(0,-1)};
          }
          return r;
        }).filter(Boolean);
      });
      ...
      setGeoZones(function(prev){
        return prev.map(function(z,i){
          if(i===activeGeoIdx){
            if(z.points.length<=1){setActiveGeoIdx(null);return null;}
            return {rock:z.rock,points:z.points.slice(0,-1)};
          }
          return z;
        }).filter(Boolean);
      });
  ...
  function placeFromCoord(ll){
    ...
    if(mode==="town"){var name=townForm.name||"Town "+(towns.length+1);setTowns(function(t){return t.concat([{lat:ll.lat,lon:ll.lon,name:name}]);});setTownForm({name:""});}
      ...
      if(activeRoadIdx===null){var nr={type:mode==="road-major"?"major":"minor",points:[ll]};setRoads(function(prev){setActiveRoadIdx(prev.length);return prev.concat([nr]);});}
      else{setRoads(function(prev){return prev.map(function(r,i){if(i===activeRoadIdx)return{type:r.type,points:r.points.concat([ll])};return r;});});}
    }
    else if(mode==="river"){
      if(activeRiverIdx===null){var nrv={points:[ll]};setRivers(function(prev){setActiveRiverIdx(prev.length);return prev.concat([nrv]);});}
      else{setRivers(function(prev){return prev.map(function(r,i){if(i===activeRiverIdx)return{points:r.points.concat([ll])};return r;});});}
    }
    else if(mode==="geology"){
      if(activeGeoIdx===null){var ng={rock:geoRock,points:[ll]};setGeoZones(function(prev){setActiveGeoIdx(prev.length);return prev.concat([ng]);});}
      else{setGeoZones(function(prev){return prev.map(function(z,i){if(i===activeGeoIdx)return{rock:z.rock,points:z.points.concat([ll])};return z;});});}
    }
  ...
  return(
    <div style={{background:"#0d0d1f",height:"100vh",fontFamily:"sans-serif",color:"#eee",display:"flex",flexDirection:"column",overflow:"hidden"}}>
      <div style={{background:"#12122e",borderBottom:"1px solid #2a2a5a",padding:"7px 14px",display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <div>
            ...
            <div style={{fontSize:9,color:"#555"}}>Nigeria Geological Survey — Sequence 5B</div>
          </div>
        </div>
        ...
        <div ref={containerRef} style={{flex:1,position:"relative",overflow:"hidden"}}>
          ...
          {mode==="select"&&(
            <div style={{position:"absolute",bottom:28,left:"50%",transform:"translateX(-50%)",background:"rgba(0,20,0,0.85)",color:"#27ae60",borderRadius:8,padding:"6px 14px",fontSize:11}}>
              Click any feature to select · then delete from sidebar
            </div>
          )}
        </div>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:10}}>
                  ...
                  <div style={{background:"#12122e",border:"1px solid #1a3a1a",borderRadius:8,padding:10}}>
                    ...
                    {selectedFeature?(
                      <div>
                        ...
                        <button onClick={deleteSelected} style={Object.assign({},btnBase,{width:"100%",background:"#3a0a0a",color:"#e74c3c",border:"1px solid #e74c3c",padding:"8px",fontSize:11,marginBottom:4})}>🗑 Delete Selected</button>
                        <button onClick={function(){setSelectedFeature(null);}} style={Object.assign({},btnBase,{width:"100%",background:"#1a1a2a",color:"#888",border:"1px solid #3a3a6a",padding:"6px",fontSize:10})}>✕ Deselect</button>
                      </div>
                    ):(
                ...
                {mode==="town"&&(
                  <div style={{background:"#12122e",border:"1px solid #1a3a5a",borderRadius:8,padding:10}}>
                    ...
                    <input value={townForm.name} onChange={function(e){setTownForm({name:e.target.value});}} placeholder="e.g. Ananamong"
                ...
                {(mode==="road-major"||mode==="road-minor")&&(
                  <div style={{background:"#12122e",border:"1px solid #5a2a00",borderRadius:8,padding:10}}>
                    ...
                    <div style={{fontSize:10,color:"#888",lineHeight:1.6}}>Click as many nodes as you need · curves through all points · finish when done</div>
                ...
                {mode==="sample"&&(
                  <div style={{background:"#12122e",border:"1px solid #5a1a1a",borderRadius:8,padding:10}}>
                    ...
                    <input value={sampleForm.id} onChange={function(e){setSampleForm(function(f){return Object.assign({},f,{id:e.target.value});});}} placeholder="e.g. UU/GS/GLG/25/57"
                    ...
                    <textarea value={sampleForm.notes} onChange={function(e){setSampleForm(function(f){return Object.assign({},f,{notes:e.target.value});});}}
                      rows={2} placeholder="Field notes…" style={{width:"100%",background:"#1e1e3a",color:"#fff",border:"1px solid #3a3a6a",borderRadius:5,padding:"6px 8px",fontSize:10,boxSizing:"border-box",resize:"none"}}/>
                    <CoordInput label="Place by Coordinates" onPlace={placeFromCoord}/>
                  </div>
                )}
                {mode==="geology"&&(
                  <div style={{background:"#12122e",border:"1px solid #2a1a5a",borderRadius:8,padding:10}}>
                    ...
                    <div style={{fontSize:10,color:"#888",lineHeight:1.6}}>Click nodes · green circle to close · min 3 points</div>
                    {activeGeo&&<div style={{marginTop:6,fontSize:10,color:"#f0c040"}}>{activeGeo.points.length} nodes placed</div>}
                    <CoordInput label="Add Node by Coordinates" onPlace={placeFromCoord}/>
                  </div>
                )}
                {mode==="pan"&&(
                  <div style={{fontSize:11,color:"#555",textAlign:"center",padding:20,lineHeight:1.8}}>Select a drawing tool above to begin.</div>
                )}
                ...
                {samples.length>0&&(
                  <div style={{background:"#12122e",border:"1px solid #2a2a5a",borderRadius:8,padding:10,marginTop:4}}>
                    ...
                    {samples.map(function(s,i){return(
                      <div key={i} style={{fontSize:10,borderBottom:"1px solid #1a1a3a",padding:"4px 0"}}>
            ...
            {tab==="output"&&(
              <div style={{display:"flex",flexDirection:"column",gap:10}}>
                ...
                <div style={{background:"#12122e",border:"1px solid #2a2a5a",borderRadius:8,padding:10}}>
                  ...
                  <div style={{fontSize:9,color:"#555",marginBottom:8,lineHeight:1.5}}>Export your map data for use in QGIS, ArcGIS, Google Earth, or Excel.</div>
                  <button onClick={function(){exportGeoJSON(towns,roads,rivers,samples,geoZones,projectName||"Study_Area");}}
                    style={Object.assign({},btnBase,{width:"100%",background:"#0a2a1a",color:"#27ae60",border:"1px solid #27ae60",padding:"8px",fontSize:10,marginBottom:5})}>
                    ⬇ GeoJSON — QGIS / ArcGIS
                  </button>
                  <button onClick={function(){exportKML(towns,roads,rivers,samples,geoZones,projectName||"Study_Area");}}
                    style={Object.assign({},btnBase,{width:"100%",background:"#0a1a2a",color:"#4a9adf",border:"1px solid #4a9adf",padding:"8px",fontSize:10,marginBottom:5})}>
                    ⬇ KML — Google Earth
                  </button>
                  <button onClick={function(){exportCSV(samples,projectName||"Study_Area");}}
                    style={Object.assign({},btnBase,{width:"100%",background:"#1a1a0a",color:"#f0c040",border:"1px solid #f0c040",padding:"8px",fontSize:10})}>
                    ⬇ CSV — Sample Data (Excel)
                  </button>
                </div>
                <div style={{background:"#0f0f1e",border:"1px dashed #2a2a4a",borderRadius:8,padding:10}}>
                  <div style={{fontSize:10,color:"#444",fontWeight:"bold",marginBottom:4}}>NEXT: SEQUENCE 6</div>
                  <div style={{fontSize:10,color:"#333",lineHeight:1.6}}>Database + user accounts (Supabase).</div>
                </div>
              </div>
            )}
          </div>
      ...
      <div style={{background:"#0a0a1e",borderTop:"1px solid #2a2a5a",padding:"4px 14px",display:"flex",justifyContent:"space-between",flexShrink:0}}>
        <span style={{fontSize:9,color:"#333"}}>Geo Mapping System v0.5B — GIS Export</span>
